...
const addOrUpdateRole = async () => {
  ...
  try {
    ...
    let res;
    if (props.roleId) {
      res = await axios.put(`${apiBase}/pharmacy-app/api/roles/${props.roleId}`, payload, config);
    } else {
...
const fetchRoleData = async () => {
  ...
  try {
    const res = await axios.get(`${apiBase}/pharmacy-app/api/roles/${route.params.id}`, config);
